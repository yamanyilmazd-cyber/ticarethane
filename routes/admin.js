...
router.get('/stats', (_req, res) => {
  try {
    ...
    res.json({
      ...
      new_users_today:     db.prepare(`SELECT COUNT(*) AS n FROM users WHERE role='user' AND DATE(created_at,'+3 hours')=DATE('now','+3 hours')`).get().n,
      new_listings_today:  db.prepare(`SELECT COUNT(*) AS n FROM listings WHERE DATE(created_at,'+3 hours')=DATE('now','+3 hours')`).get().n,
